...
export default function PremiumPropertiesScroller() {
  ...
  useEffect(() => {
    const fetchPremiumProperties = async () => {
      try {
        const response = await fetch("/api/properties?premium=true&limit=12");
        const data = await response.json();
        setProperties(data.properties || []);
      } catch (err) {
        console.error("Failed to fetch premium properties:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Exclusive high-value properties for discerning buyers</p>
          </div>
          <a
            href="/properties"
